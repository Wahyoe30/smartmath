            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h2>Daftar Quiz</h2>
                        <a href="{{ route('quizs.create') }}" class="btn btn-primary float-right">Tambah</a>
                    </div>
                    <div class="card-body">
                        @if (session('success'))
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                @foreach ($quizs as $q)
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $q->materi->title }}</td>
                                        <td>{{ $q->questions }}</td>
                                        <td>{{ $q->answer }}</td>
                                        <td>
                                            {{-- <a href="{{ route('users.show', $user->id) }}" class="btn btn-info btn-sm">Detail</a> --}}
                                            <a href="{{ route('quizs.edit', $q->id) }}" class="btn btn-warning btn-sm">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('quizs.destroy', $q->id) }}" method="POST"
